<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../../../mvc/assets/css/danhmuc.css">
    <title>Document</title>
</head>
<body>
    <form action="" method="POST">
    <div class="main-dm">
        <span>Danh mục của bạn</span>
        <div id="qly-danhmucnha">
            <table>
                <thead>
                    <tr>
                        <th>STT</th>
                        <th>Hình ảnh</th>
                        <th>Tên danh mục</th>
                        <th>Mô tả</th>
                        <th>Sửa</th>
                        <th>Xóa</th>
                    </tr>
                </thead>
                <tbody id="danhmuc-item">
                </tbody>
            </table>
        </div>
        <span id="null">(Danh mục trống)</span>
        <div class="page" id="nb-page">
        </div>
    </div>
    </form>
    <div id="modal_sua">
        <form action="../../../cAdmin/admin/dm/all" method="POST" enctype="multipart/form-data" id="form-suadm">
            <div id="modal_overlay">
            <div id="modalsua_body">
                <h3>Sửa danh mục</h3>
                    <div class="form-error" id="form-error">
                        <i class="fas fa-exclamation-circle"></i>
                        <span id="message-error">dd</span>
                    </div>
                    <div id="them-dm">
                        <div id="inf-add">
                            <div class="item-add">
                                <label for="">Tên danh mục: </label>
                                <input id="tendm-sua" name="tendm-sua" type="text">
                            </div>
                            <div class="item-add">
                                <label for="">Mô tả: </label>
                                <textarea name="mota-sua" id="mota-sua" cols="30" rows="5"></textarea>
                            </div>
                        </div>
                        <input id="iddm-sua" name="iddm-sua" type="hidden">
                        <input id="imgcune" name="imgcu" type="hidden">
                        <div id="anhdanhmuc">
                            <label for="">Ảnh mô tả</label>
                            <img id="img-dm-sua" src="data:image/jpeg;base64," alt="">
                            <input type="file" name="img-sua" id="img-sua" onchange="choose(event)">
                        </div>
                    </div>
                <div id="btn">
                    <button type="button" id="xoamodal" onclick="update()">Lưu</button>
                    <button type="button" id="huydel" onclick="closeModaldel()">Hủy</button>
                </div>
            </div>
            </div>
        </form>
    </div>
    <div id="modal_xoanha">
        <form action="../../../cAdmin/admin/dm/all" method="POST" id="form-xoadm">
            <div id="modal_overlay">
            <div id="modalxoa_body">
                <h3>Xóa danh mục</h3>
                <p>Xóa danh mục sẽ xóa tất cả dữ liệu liên quan đến danh mục này (hình ảnh, phòng..)</br><b>Bạn chắc chắn muốn xóa?</b></p>
                <input id="iddm-xoa" name="iddm-xoa" type="hidden">
                <div id="btn">
                    <button type="button" id="xoamodal" onclick="xoa()">Xóa</button>
                    <button type="button" id="huydel" onclick="closeModalxoa()">Hủy</button>
                </div>
            </div>
            </div>
        </form>
    </div>
</body>
</html>
<script type="text/javascript">
    window.onload = function() {
        loadnbpagedm();
    }

    //onchange="choose(event)"
    function choose(event){
        var output = document.getElementById("img-dm-sua");
        var reader = new FileReader();
        reader.onload = function(){
            output.src = reader.result;
        }
        reader.readAsDataURL(event.target.files[0]);
    }

    // $(document).ready(function(){
    // });

    function loadnbpagedm(){
        $.ajax({
			type: "GET",
			dataType: 'text',
			url: './../../../cdanhmuc/countDM' ,
			success: function(json){
                var page = JSON.parse(json) / 5;
                var n = Math.round(page);
                if(n < page){
                    n = n + 1;
                }
                $('#nb-page').empty();
                $('#nb-page').append(`
                    <a type="button">«</a>
                `);
                for(let j = 1; j <= n; j++){
                    if(j == 1){
                        $('#nb-page').append(`
                            <a type="button" class="btn active">${j}</a>
                        `);
                        loaddm();
                    }else{
                        $('#nb-page').append(`
                            <a type="button" class="btn">${j}</a>
                        `);
                    }
                }
                $('#nb-page').append(`
                    <a type="button">»</a>
                `);
                var header = document.getElementById("nb-page");
                var btns = header.getElementsByClassName("btn");
                for (var i = 0; i < btns.length; i++) {
                    btns[i].addEventListener("click", function() {
                        var current = document.getElementsByClassName("active");
                        current[0].className = current[0].className.replace(" active", "");
                        this.className += " active";
                        loaddm();
                    });
                }
			},
			error: function(error) {
				console.log(error);
			}
        });
    }

    function loaddm(){
        $('#danhmuc-item').empty();
        var active = document.getElementsByClassName("active");
        var count = 5;
        var start = ( active[0].innerText * count) - count;
        $.ajax({
			type: "POST",
            data: {'start': start, 'count': count},
			dataType: 'text',
			url: './../../../cdanhmuc/getdanhmucP' ,
			success: function(json){
				var danhmuc = JSON.parse(json);
                console.log(danhmuc);
                if(danhmuc.length != 0){
                    document.getElementById("null").style.display = "none";
                    var stt = start + 1;
                    for( let i = 0; i < danhmuc.length; i++ ){
                        $('#danhmuc-item').append(`<tr>
                            <td id="stt">${stt}</td>
                            <td id="tendm"><img id="img-danhmuc" src="../../../mvc/assets/img/${danhmuc[i]['img']}" alt=""></td>
                            <td id="tendm">${danhmuc[i]['tendm']}</td>
                            <td id="mota">${danhmuc[i]['mota']}</td>
                            <td><div class="edit" id="${danhmuc[i]['madm']}" onclick="suadanhmuc(${danhmuc[i]['madm']})"><img class="icon" src="../../../mvc/assets/img/edit.svg" alt=""></div></td>
                            <td><div class="edit" id="${danhmuc[i]['madm']}" onclick="xoadanhmuc(${danhmuc[i]['madm']})"><img class="icon" src="../../../mvc/assets/img/bin.svg" alt=""></div></td>
                        </tr>`);
                        stt++;
                    }
                }else{
                    document.getElementById("null").style.display = "block";
                    document.getElementById("qly-danhmucnha").style.display = "none";
                }
			},
			error: function(error) {
				console.log(error);
			}
        });
    }

    function suadanhmuc(id){
        $.ajax({
			type: "POST",
            data: {'iddm': id},
			dataType: 'text',
			url: './../../../cdanhmuc/getdanhmucupdate' ,
			success: function(json){
				var danhmuc = JSON.parse(json);
                $('#tendm-sua').val(danhmuc['tendm']);
                $('#mota-sua').val(danhmuc['mota']);
                $('#iddm-sua').val(danhmuc['madm']);
                $('#imgcune').val(danhmuc['img']);
                $('#img-dm-sua').attr('src',  "../../../mvc/assets/img/" + danhmuc['img']);
                var modal = document.getElementById("modal_sua");
                modal.style.display='block';
			},
			error: function(error) {
				console.log(error);
			}
        });
    }

    function closeModaldel(){
        var modal = document.getElementById("modal_sua");
        modal.style.display='none';
    }

    function xoadanhmuc(id){
        $.ajax({
			type: "POST",
            data: {'iddm': id},
			dataType: 'text',
			url: './../../../cdanhmuc/getdanhmucupdate' ,
			success: function(json){
				var danhmuc = JSON.parse(json);
                $('#iddm-xoa').val(danhmuc['madm']);
                var modal = document.getElementById("modal_xoanha");
                modal.style.display='block';
			},
			error: function(error) {
				console.log(error);
			}
        });
    }

    function closeModalxoa(){
        var modal = document.getElementById("modal_xoanha");
        modal.style.display='none';
    }

    function update(){
        let tendm = $('#tendm-sua').val();
		let mota = $('#mota-sua').val();
        let iddm = $('#iddm-sua').val();
        let imgcune = $('#imgcune').val();
        var file_data = $('#img-sua')[0].files[0];
        var form_data = new FormData();                  
        form_data.append('tendm', tendm);
        form_data.append('motane', mota);
        form_data.append('iddm', iddm);
        form_data.append('imgcune', imgcune);
        form_data.append('img-sua', file_data);
		$.ajax({
				type: "POST",
                data: form_data,
                cache: false,
                contentType: false,
                processData: false,
				dataType: 'text',
				url: './../../../cdanhmuc/updatedm' ,
				success: function(json){
					let result = JSON.parse(json);
					if(result === 'tendm'){
						document.getElementById('form-error').style.display = 'block';
						$('#message-error').text("Tên danh mục bị trùng, vui lòng nhập tên khác!");
					}else if(result === 'fail'){
						document.getElementById('form-error').style.display = 'block';
						$('#message-error').text("Xảy ra lỗi, Vui lòng thử lại!");
					}else if(result === 'null'){
						document.getElementById('form-error').style.display = 'block';
						$('#message-error').text("Vui lòng nhập đầy đủ thông tin !");
					}else{
						document.getElementById("modal_sua").style.display='none';
                        loaddm();
					}
				},
				error: function(error) {
					console.log(error);
				}
		});
    }

    function xoa(){
        let iddm = $('#iddm-xoa').val();
        var form_data = new FormData();                  
        form_data.append('iddm', iddm);
		$.ajax({
				type: "POST",
                data: form_data,
                cache: false,
                contentType: false,
                processData: false,
				dataType: 'text',
				url: './../../../cdanhmuc/deletedm' ,
				success: function(json){
					document.getElementById("modal_xoanha").style.display='none';
                    loaddm();
				},
				error: function(error) {
					console.log(error);
				}
		});
    }
</script>